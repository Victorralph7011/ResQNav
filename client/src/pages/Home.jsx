...
function StickyMapSequence() {
  ...
  // Act transitions — wide hold zones so nothing vanishes mid-scroll
  const act1Opacity = useTransform(smoothProgress, [0, 0.25, 0.30], [1, 1, 0]);
  const act2Opacity = useTransform(smoothProgress, [0.25, 0.30, 0.60, 0.65], [0, 1, 1, 0]);
  const act3Opacity = useTransform(smoothProgress, [0.65, 0.75, 1.0], [0, 1, 1]);

  // Act 1 — pulsing dots
  const dotsScale = useTransform(smoothProgress, [0, 0.25], [0.8, 1]);

  // Act 2 — card slide (enters within act2's visible range)
  const cardY = useTransform(smoothProgress, [0.25, 0.35], [60, 0]);
  const cardOpacity = useTransform(smoothProgress, [0.25, 0.35, 0.60, 0.65], [0, 1, 1, 0]);

  // Act 3 — route drawing
  const routeLength = useTransform(smoothProgress, [0.70, 0.92], [0, 1]);
  const routeGlow = useTransform(smoothProgress, [0.78, 0.90, 1.0], [0, 1, 1]);

  // Act-specific text
  const text1Y = useTransform(smoothProgress, [0, 0.08], [30, 0]);
  const text1O = useTransform(smoothProgress, [0, 0.08, 0.25, 0.30], [0, 1, 1, 0]);
  const text3Y = useTransform(smoothProgress, [0.65, 0.75], [30, 0]);
  const text3O = useTransform(smoothProgress, [0.65, 0.75, 1.0], [0, 1, 1]);
  ...
  return (
    <section ref={containerRef} className="relative h-[300vh]">
      <div className="sticky top-0 h-screen w-full overflow-hidden flex items-center justify-center">
        ...
        {/* ─── ACT 1: The Gridlock ─── */}
        <motion.div className="absolute inset-0 flex items-center justify-center" style={{ opacity: act1Opacity }}>
          {/* Traffic dots */}
          <motion.div className="absolute inset-0" style={{ scale: dotsScale }}>
          ...
          {/* Act 1 text */}
          <motion.div className="relative z-10 text-center px-6" style={{ y: text1Y, opacity: text1O }}>
            <h2 className="text-[8vw] sm:text-[6vw] font-bold text-white tracking-[-0.05em] leading-[0.9]">
              Cities are choking.
            </h2>
            <p className="text-zinc-600 text-[14px] sm:text-[16px] mt-4 tracking-wide">12.4 billion hours lost to traffic annually</p>
          </motion.div>
        </motion.div>

        {/* ─── ACT 2: The Alert ─── */}
        <motion.div className="absolute inset-0 flex items-center justify-center" style={{ opacity: act2Opacity }}>
          {/* Frozen dots — dimmed */}
          ...
          >
            <div className="backdrop-blur-xl bg-white/[0.04] border border-white/[0.1] rounded-2xl p-8 shadow-2xl">
              {/* Status badge */}
              <div className="flex items-center gap-2 mb-6">
                <span className="w-2.5 h-2.5 rounded-full bg-white animate-pulse" />
                <span className="text-[11px] font-bold text-white/80 uppercase tracking-[0.15em]">Live Alert</span>
              </div>

              <h3 className="text-[22px] font-semibold text-white tracking-tight mb-2">
                AI Verified Incident Reported
              </h3>
              <p className="text-[14px] text-zinc-500 leading-relaxed mb-6">
                Multi-vehicle collision detected on NH-48 near Sector 21. Emergency services dispatched. Commuter rerouting initiated.
              </p>
              ...
              <div className="flex items-center gap-6 text-[12px] text-zinc-600">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span>NH-48, Sector 21</span>
                </div>
                <span className="ml-auto text-white/40 font-semibold">Severity: Critical</span>
              </div>

              {/* Services dispatched */}
              <div className="mt-6 pt-4 border-t border-white/[0.06] flex items-center gap-3">
                {['Ambulance', 'Fire', 'Police'].map((s) => (
                  <span key={s} className="text-[10px] font-semibold text-zinc-500 bg-white/[0.04] border border-white/[0.06] px-2.5 py-1 rounded-full">{s}</span>
                ))}
              </div>
            </div>
          </motion.div>
        ...
        {/* ─── ACT 3: The Corridor ─── */}
        <motion.div className="absolute inset-0" style={{ opacity: act3Opacity }}>
          {/* Route SVG */}
          ...
          {/* Cleared dots swept away */}
          <motion.div className="absolute inset-0" style={{ opacity: useTransform(smoothProgress, [0.7, 0.9], [0.08, 0.02]) }}>
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <motion.div className="relative z-10 text-center px-6" style={{ y: text3Y, opacity: text3O }}>
              <h2 className="text-[8vw] sm:text-[6vw] font-bold text-white tracking-[-0.05em] leading-[0.9]">
                We clear the path.
              </h2>
              <p className="text-zinc-600 text-[14px] sm:text-[16px] mt-4 tracking-wide">Priority corridor activated in 8 seconds</p>
            </motion.div>
          </div>
          ...
          <motion.div
            ...
            style={{ opacity: routeGlow }}
          >
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/[0.04] border border-white/[0.1] backdrop-blur-sm">
              <span className="w-2 h-2 rounded-full bg-white" />
              <span className="text-[11px] font-bold text-white uppercase tracking-[0.12em]">Corridor Active</span>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/[0.04] border border-white/[0.1] backdrop-blur-sm">
              <span className="text-[11px] font-semibold text-zinc-500 tracking-wide">ETA: 4 min</span>
            </div>
          </motion.div>
        </motion.div>
